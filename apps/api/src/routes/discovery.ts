...
export const discoveryRoutes: FastifyPluginAsync = async (app) => {
  app.get('/feed/following', { preHandler: requireUser }, async (request) => {
    ...
    const result = await pool.query(
      `${entrySelect} JOIN follows f ON f.following_id=e.user_id AND f.follower_id=$1 WHERE e.deleted_at IS NULL AND pinory_entry_visible(e.user_id,e.visibility,$1) AND ($2::timestamptz IS NULL OR e.created_at<$2) ORDER BY e.created_at DESC LIMIT $3`,
  ...
  app.get('/feed/nearby', { preHandler: requireUser }, async (request) => {
    ...
    const result = await pool.query(
      `${entrySelect} WHERE e.deleted_at IS NULL AND e.visibility='PUBLIC' AND ($1::timestamptz IS NULL OR e.created_at<$1) AND (($2::text IS NOT NULL AND p.city ILIKE $2) OR ($2::text IS NULL AND ST_DWithin(p.location,ST_SetSRID(ST_MakePoint($4,$3),4326)::geography,50000))) ORDER BY p.popularity_score DESC,e.created_at DESC LIMIT $5`,
  ...
  app.get('/feed/global', { preHandler: requireUser }, async (request) => {
    const q = feedQuery.parse(request.query);
    const result = await pool.query(
      `${entrySelect} WHERE e.deleted_at IS NULL AND e.visibility='PUBLIC' AND ($1::timestamptz IS NULL OR e.created_at<$1) ORDER BY (p.popularity_score + e.comments_count*2 + e.views_count*.2) DESC,e.created_at DESC LIMIT $2`,
  ...
  app.get('/places/:id/entries', { preHandler: requireUser }, async (request) => {
    ...
    const result = await pool.query(
      `${entrySelect} WHERE e.place_id=$1 AND e.deleted_at IS NULL AND pinory_entry_visible(e.user_id,e.visibility,$2) ORDER BY e.created_at DESC LIMIT 100`,
      [id, request.user.sub],
    );
