...
import { Camera, Compass, Feather, Layers3, Map, MapPin, Route, Sparkles, Users } from 'lucide-react';
import type { AchievementsResponse } from '../lib/api';

const icons = { pin: MapPin, compass: Compass, map: Map, sparkles: Sparkles, feather: Feather, camera: Camera, users: Users, layers: Layers3, route: Route };
...
export function AchievementGallery({ data, isOwn }: { data: AchievementsResponse; isOwn: boolean }) {
  ...
  return <div className="achievement-gallery">
    <motion.div className="level-card" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
      ...
      <div><span className="eyebrow">УРОВЕНЬ ИССЛЕДОВАТЕЛЯ</span><h3>{levelTitle(data.level)}</h3><p>{data.totalXp} XP собрано</p></div>
