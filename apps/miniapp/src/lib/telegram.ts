...
export interface TelegramAdapter {
  ...
  location(): Promise<{ lat: number; lng: number; accuracy?: number }>;
    ...
    async location() {
      if (Capacitor.isNativePlatform()) {
        ...
        if (permissions.location === 'prompt' || permissions.location === 'prompt-with-rationale') {
          ...
          if (requested.location !== 'granted') throw new Error('Разрешите Pinory доступ к местоположению');
        } else if (permissions.location !== 'granted') {
          throw new Error('Доступ к местоположению отключён в настройках Android');
        }
      ...
      return new Promise((resolve, reject) => {
        const fallback = () => navigator.geolocation
        ...
        if (webApp?.LocationManager) {
          webApp.LocationManager.init(() => webApp.LocationManager!.getLocation((location) => location
